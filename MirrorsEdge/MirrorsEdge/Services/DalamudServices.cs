...
{
                    internal MirrorsEdgePlugin              MirrorsEdgePlugin           { get; private set; } = null!;
    ...
    [PluginService] internal ISigScanner                    SigScanner                  { get; private set; } = null!;

    public static DalamudServices Create(IDalamudPluginInterface plugin, MirrorsEdgePlugin mirrorsEdge)
    {
        DalamudServices service = new DalamudServices();

        _ = plugin.Inject(service); // Cannot log anyways if injection fails :shrug:

        service.MirrorsEdgePlugin   = mirrorsEdge;
