...
public class ShaderFactory
{
    ...
    public ShaderFactory(MirrorServices mirrorServices)
    {
    ...
    public bool GetVertexShader(string resourceName, InputElement[] inputElements, [NotNullWhen(true)] out VertexShader? vertexShader, out InputLayout? inputLayout, [NotNullWhen(true)] out SamplerState? samplerState)
    {
        ...
        try
        {
            ...
            vertexShader = new VertexShader(MirrorServices.DirectXData.Device, vertexShaderResult);

            if (inputElements.Length > 0)
            {
                inputLayout = new InputLayout(MirrorServices.DirectXData.Device, ShaderSignature.GetInputSignature(vertexShaderResult), inputElements);
            }

            samplerState = new SamplerState(MirrorServices.DirectXData.Device, SamplerDescription);
    ...
    public bool GetFragmentShader(string resourceName, [NotNullWhen(true)] out PixelShader? fragmentShader)
    {
        ...
        try
        {
            ...
            fragmentShader = new PixelShader(MirrorServices.DirectXData.Device, fragmentShaderResult);
    ...
    private byte[] LoadShaderBytes(string resourceName)
    {
        ...
        return MirrorServices.ResourceLoader.GetEmbeddedResourceBytes(resourceName);
    }
}